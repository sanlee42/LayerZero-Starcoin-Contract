...
module msglib_auth::msglib_cap {
    ...
    use layerzero_common::acl_std::{Self, ACL};
    ...
    public fun new_version<MGSLIB>(account: &signer, major: bool): (SemVer, MsgLibSendCapability, MsgLibReceiveCapability) acquires GlobalStore {
        ...
        acl_std::assert_contains(&store.msglib_acl, address_of(account));
        // remove from ACl after registration
        acl_std::remove(&mut store.msglib_acl, address_of(account));
        ...
        store.last_version = copy version;
        (copy version, MsgLibSendCapability { version: copy version }, MsgLibReceiveCapability { version })
    }
    //FIXME: entry fun
    public fun allow(account: &signer, msglib_receive: address) acquires GlobalStore {
        assert_signer(account, @msglib_auth);

        let store = borrow_global_mut<GlobalStore>(@msglib_auth);
        acl_std::add(&mut store.msglib_acl, msglib_receive);
    }
    
    //FIXME: entry fun
    public fun disallow(account: &signer, msglib_receive: address) acquires GlobalStore {
        assert_signer(account, @msglib_auth);

        let store = borrow_global_mut<GlobalStore>(@msglib_auth);
        acl_std::remove(&mut store.msglib_acl, msglib_receive);
    }
    ...
    public fun assert_send_version(cap: &MsgLibSendCapability, major: u64, minor: u8) {
        ...
        assert!(cap_major == major && cap_minor == minor, Errors::invalid_argument(EMSGLIB_VERSION_NOT_SUPPORTED));
    }
