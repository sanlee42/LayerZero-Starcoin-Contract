module layerzero_common::packet {
    ...
    public fun decode_dst_address(packet_bytes: &vector<u8>, src_address_size: u64): vector<u8> {
        vector_slice(packet_bytes, 12 + src_address_size, 28 + src_address_size)
    }

    // only apply to serialized packet of this specific packet type
    public fun decode_payload(packet_bytes: &vector<u8>, src_address_size: u64): vector<u8> {
        // starcoin address is 16 bytes. so start from 28 
        vector_slice(packet_bytes, 28 + src_address_size, Vector::length(packet_bytes))
    }
    ...
    #[test_only]
    use StarcoinFramework::Compare::{is_equal, cmp_bytes};
    #[test_only]
    ...
    fun test_encode_packet() {
        ...
        assert!(is_equal(cmp_bytes(&encoded, &vector<u8>[0, 0, 0, 0, 0, 0, 18, 52, 0, 1, 1, 2, 3, 4, 5, 6, 0, 2, 7, 8, 9, 10, 11, 12])), 1);
    }
    ...
    fun test_hash_packet() {
        ...
        assert!(is_equal(cmp_bytes(&hash, &vector<u8>[160, 33, 246, 191, 58, 98, 150, 157, 194, 101, 84, 32, 27, 37, 210, 161, 174, 187, 155, 55, 97, 162, 32, 68, 218, 115, 15, 113, 10, 17, 117, 68])), 1);
    }
