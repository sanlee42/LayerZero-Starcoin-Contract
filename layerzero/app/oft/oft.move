module layerzero::oft {
    ...
    use StarcoinFramework::Token::{Self, BurnCapability, MintCapability, Token};
    ...
    public fun init_base_oft<OFT: store>(account: &signer): UaCapability<OFT> {
        init<OFT>(account, true)
    }

    public fun init_oft<OFT: store>(
        account: &signer,
        mint_cap: MintCapability<OFT>,
        burn_cap: BurnCapability<OFT>,
    ): UaCapability<OFT> {
    ...
    //
    public entry fun send<OFT: store>(
        sender: &signer,
        dst_chain_id: u64,
        dst_receiver: vector<u8>,
        amount: u128,
        ...
        let coin = Account::withdraw<OFT>(sender, amount);
        let fee = Account::withdraw<STC>(sender, fee);

        let refund = send_coin(sender_addr, coin, dst_chain_id, dst_receiver, fee, adapter_params, msglib_params);

        // refund to sender
        Account::deposit(sender_addr, refund);
    }

    public fun send_coin<OFT: store>(
        ...
        adapter_params: vector<u8>,
        msglib_params: vector<u8>,
    ): Token<STC> acquires LockedCoin, GlobalStore, CoinCapabilities, EventStore {
        assert_oft_initialized<OFT>();

        let oft_address = type_address<OFT>();
        let global_store = borrow_global<GlobalStore<OFT>>(oft_address);

        let amount = Token::value<OFT>(&coin);
        if (global_store.base) {
            // lock the coin
            let locked_coin = borrow_global_mut<LockedCoin<OFT>>(oft_address);
            Token::join<OFT>(locked_coin.total_coin, coin);
        } else {
            // burn the coin
            let caps = borrow_global<CoinCapabilities<OFT>>(oft_address);
            Token::burn_with_capability<OFT>(&caps.burn_cap, coin);
        };

        // send lz msg
        let payload = encode_send_payload(sender_address, dst_receiver, amount as u64);
    ...
    public entry fun lz_receive<OFT: store>(
        src_chain_id: u64,
        ...
        // try to deposit
        let locked_coin = borrow_global_mut<LockedCoin<OFT>>(oft_address);
        let stashed = !Token::is_registered_in<OFT>(receiver);
        if (stashed) {
    ...
    public entry fun claim<OFT>(receiver: &signer) acquires EventStore, LockedCoin {
        ...
        let oft_address = type_address<OFT>();
        let locked_coin = borrow_global_mut<LockedCoin<OFT>>(oft_address);
        assert!(table::contains(&locked_coin.claimable_amount, receiver_addr), Errors::not_found(EOFT_CLAIMABLE_COIN_NOT_FOUND));

        let claimable = table::remove(&mut locked_coin.claimable_amount, receiver_addr);
        assert!(claimable > 0, Errors::not_found(EOFT_CLAIMABLE_COIN_NOT_FOUND));
    ...
    // internal functions
    //
    fun init<OFT: store>(account: &signer, base: bool): UaCapability<OFT> {
        assert_type_signer<OFT>(account);
        assert!(
            !is_oft_initialized<OFT>(),
            Errors::already_published(EOFT_ALREADY_INITIALIZED),
        );
    ...
    fun assert_oft_initialized<OFT>() {
        assert!(is_oft_initialized<OFT>(), Errors::not_published(EOFT_NOT_INITIALIZED));
    }
    ...
    fun decode_receive_payload(payload: &vector<u8>): (vector<u8>, address, u64) {
        ...
        assert!(packet_type == PT_RECEIVE, Errors::internal(EOFT_INVALID_PACKET_TYPE));
